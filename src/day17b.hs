...
neighbours m p@(_,d,n) = map (\w@((a,b),_,n) -> (w,(m!!a)!!b)) valids
  where
    valids = filter (\((a,b),nd,nn) -> a>=0 && a<u && b>=0 && b<v && (n>=4 || d==nd) && nn<=10) $ newDirs p
    u = length m
    v = length $ head m

final :: Mapa -> Point -> Bool
final m ((a,b),_,n) = ((length m)-1)==a && ((length $ head m)-1)==b && n>=4
...
solve :: T.Text -> Int
solve x = ff + (minimum $ map (\w -> fst $ fromJust w) d)
  where
    ff = head $ head mapa
